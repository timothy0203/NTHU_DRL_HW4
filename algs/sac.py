...
def learn(env, exploration, seed, n_total_steps, max_episode_length, alg_args, args):
    ...
    print("ready to reset env in sac") # TODO: debug
    obs = env.reset()
    print("ready to start play in sac") # TODO: debug
    ...
    for t in range(start_step, n_total_steps + start_step):
        ...
        if t % args.save_interval == 0:
            print(f"save model: {np.mean(episode_lengths_history)}")
            saver.save(sess, args.output_dir + f'/agent{int(np.mean(episode_lengths_history))}.ckpt', global_step=tf.train.get_global_step())
